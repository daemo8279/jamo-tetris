...
import { toggleRemove, toggleGem, updateHintPanel } from './hud.js';
import {
  moveLeft, moveRight, hardDrop, setSoftDrop, doHold,
  eraseCell, eraseRow, togglePause, advanceYes, advanceNo, quitToHome
} from '../core/game.js';
import {
...
function bindKeyboard() {
  document.addEventListener('keydown', (e) => {
    ...
    switch (e.key) {
      ...
      case 'g': case 'G': toggleGem(); break;
      case 'Escape':
        if (S.removeMode) toggleRemove();
        else if (S.gemMode) toggleGem();
        break;
      default: break;
    }
  });
...
function bindCanvas() {
  ...
  canvas.addEventListener('click', (e) => {
    if (!S.removeMode && !S.gemMode) return;
    const { row, col } = cellFromPointer(e.clientX, e.clientY);
    if (S.gemMode) eraseRow(row);
    else eraseCell(row, col);
  });

  canvas.addEventListener('mousemove', (e) => {
    if (!S.removeMode && !S.gemMode) {
      if (S.hoverCell || S.hoverRow !== null) {
        S.hoverCell = null;
        S.hoverRow = null;
        draw();
      }
    ...
    if (S.gemMode) {
      S.hoverRow = inside ? row : null;
      S.hoverCell = null;
    } else {
      S.hoverCell = inside ? { row, col } : null;
      S.hoverRow = null;
    }
    draw();
  });
