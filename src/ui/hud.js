...
import { levelConfig, stageLabel, COMBO_PER_GEM, MAX_GEMS } from '../config/levels.js';
...
/** 보석 개수와 사용 버튼을 그립니다. 보석이 없으면 자리를 차지하지 않습니다. */
export function updateGems() {
  const row = $('gems-row');
  if (!row) return;
  row.innerHTML = '';
  row.style.display = S.gems > 0 ? '' : 'none';

  for (let i = 0; i < Math.min(S.gems, MAX_GEMS); i += 1) {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = `gem-btn${S.gemMode ? ' on' : ''}`;
    btn.textContent = '💎';
    btn.title = '보석으로 한 줄 지우기';
    btn.addEventListener('click', toggleGem);
    row.appendChild(btn);
  }
}

/** 보석 사용 모드를 켜고 끕니다. */
export function toggleGem() {
  if (S.gameOver || S.clearing || S.paused || S.awaitingAdvance) return;
  if (!S.gems) return;
  S.gemMode = !S.gemMode;
  S.removeMode = false;
  S.hoverCell = null;
  setCursor(S.gemMode ? 'crosshair' : 'default');
  setMsg(S.gemMode ? '💎 지울 줄을 누르세요' : '');
  updateStars();
  updateGems();
  draw();
}

/** 연속 완성 상태를 보여 줍니다. 이어지는 중일 때만 나타납니다. */
export function updateCombo() {
  ...
  chip.style.display = '';
  const toward = S.combo % COMBO_PER_GEM;
  chip.textContent = `🔥 ${S.combo} · 💎 ${toward}/${COMBO_PER_GEM}`;
  chip.classList.toggle('hot', toward === COMBO_PER_GEM - 1);
}
