 ...
 *              목표 블록은 순서대로가 아니라 섞어서 나오므로, 어디에 놓을지 스스로 판단해야 합니다.
 *   filler     연속 구간 사이에 끼워 넣는 방해 블록 수의 범위입니다.
 *   noise      방해 블록 가운데 목표·예고 낱말과 아무 상관 없는 글자가 나올 확률입니다.
 *              0이면 나중에 쓸모가 생기는 글자만, 1에 가까울수록 순수한 방해가 늘어납니다.
 *   goal       다음 레벨로 넘어가는 데 필요한 낱말 수입니다.
 *   hintEvery  블록 몇 개마다 힌트 단계를 올릴지 정합니다.
 */

/** 1~5단계의 방해 블록 비율입니다. 레벨이 오를수록 쓸모없는 글자가 늘어납니다. */
const SYL_NOISE = [0.15, 0.3, 0.4, 0.5, 0.55];
...
export function levelConfig(level) {
  if (level <= 5) {
    return {
      ...
      chunk: level <= 2 ? [2, 2] : [1, 2],
      filler: level === 1 ? [1, 1] : level <= 3 ? [1, 2] : [2, 2],
      noise: SYL_NOISE[level - 1],
      goal: goalOf(level),
      hintEvery: 4
    };
  }
  ...
  if (level <= 15) {
    return {
      ...
      filler: level <= 8 ? [1, 1] : level <= 12 ? [1, 2] : [2, 2],
      noise: level <= 8 ? 0.2 : level <= 12 ? 0.3 : 0.4,
      goal: goalOf(level),
      hintEvery: 6
    };
  }
...
/** 보석 하나를 받는 데 필요한 연속 완성 수입니다. */
export const COMBO_PER_GEM = 5;

/** 한꺼번에 지닐 수 있는 보석 수입니다. */
